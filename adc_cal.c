...
static inline int s_run           (int a_drs_num, uint32_t a_cal_flags, adc_calibrate_params_t* a_params );
...
int drs_cal_get_x(drs_t * a_drs, double * a_x, int a_flags)
{
    ...
    if (! (a_flags & DRS_CAL_APPLY_NO_CUT)){
        ...
        if (g_adc_data_cut_from_begin)
            memmove(a_x, a_x + g_adc_data_cut_from_begin, (DRS_CELLS_COUNT_CHANNEL - g_adc_data_cut_from_begin)* sizeof (double)  );

        if (g_drs_data_cut_from_end){
            size_t l_end_count = DRS_CELLS_COUNT_CHANNEL - g_adc_data_cut_from_begin;
...
void drs_cal_y_apply(drs_t * a_drs, unsigned short *a_in,double *a_out, int a_flags)
{
    ...
    if (! (a_flags & DRS_CAL_APPLY_NO_CUT)){
        size_t l_zero_count = (g_drs_data_cut_from_end + g_adc_data_cut_from_begin) * DRS_CHANNELS_COUNT;
        if (g_adc_data_cut_from_begin){
            memmove(a_out, a_out + DRS_CHANNELS_COUNT* g_adc_data_cut_from_begin, (DRS_CELLS_COUNT - l_zero_count)* sizeof (double)  );
        }
